...
- **Page-Transitions auf Desktop**: fünf Iterationen während des Polish-Passes durchlaufen, Endstand seit Mai-2026 = **vertikaler Y-Slide + Cross-Fade**, 200ms, `cubic-bezier(0.22, 1, 0.36, 1)`. Keyframes: `pageEnterDesktop` / `pageEnterDesktopPrev` (16px Y-Translate aus Richtung + Opacity 0→1), `pageLeaveDesktop` / `pageLeaveDesktopPrev` (8px in Gegenrichtung + Opacity 1→0). Direction-aware: next = Entering kommt von unten / Leaving geht nach oben weg; prev = umgekehrt. translate3d + opacity = GPU-only (kein Text-Re-Rastern auf großen Monitoren).
  - **Iteration 1** (280ms iOS-Vollscreen-Vertikal-Slide, Original): cinematic aber bremste nach 20× Wechsel.
  - **Iteration 2** (120ms reiner Opacity-Fade): zu subtil, kein Richtungs-Cue, User las als „kein Übergang".
  ...
  - **Iteration 4** (220ms horizontaler Slide ohne Scale): fixte das Wackeln, aber 40px horizontale Bewegung sind auf 4K-Displays proportional minimal und der Content-Spalten-Cap (1280px) auf einem 3840er-Screen rutschte einsam in einer Whitespace-Wüste herum.
  - **Iteration 5 (current)**: vertikal. Bewegt sich entlang der natürlichen Lese-Achse, 16px bleiben auf jedem Monitor proportional sichtbar, Content-Spalte bewegt sich konsistent zur Spalten-Form. Asymmetrische Distanzen (Entering 16px, Leaving 8px in Gegenrichtung) geben subtilen Tiefen-Cue ohne Scale.
...
25. **Desktop-Page-Animation seit Polish-Pass-Iteration 5 = vertikaler Y-Slide + Cross-Fade** (`pageEnterDesktop` / `pageEnterDesktopPrev` / `pageLeaveDesktop` / `pageLeaveDesktopPrev`, 200ms, `cubic-bezier(0.22, 1, 0.36, 1)`). Vertikaler 16px-Y-Translate auf kommender Page + Opacity 0→1; leavende Page 8px in Gegenrichtung + Opacity 1→0. Direction-aware: `data-direction="prev"` spiegelt die Y-Vorzeichen (Entering kommt dann von oben statt unten), `data-direction="none"` deaktiviert die Animation komplett (initial-load, Layout-Wechsel). KEIN scale() — Iteration 3 hatte das probiert, verursachte auf 27"+ Displays sichtbares Text-Wackeln durch Re-Rasterung der Buchstaben-Glyphs pro Frame. Iteration 4 mit horizontalem Slide ohne Scale war smooth aber konzeptuell falsch (Content-Spalte rutschte in Whitespace-Wüste auf breiten Screens). Die JS-Sequenz in `activateDesktopPage` ist kritisch: `data-direction` setzen VOR `.active-page`-Klasse hinzufügen, plus `void incoming.offsetWidth` für Force-Reflow zwischen `classList.remove` und `add` damit die Animation auch bei Re-Navigation auf dieselbe Page neu feuert. Wer die Animation deaktivieren will: `data-direction="none"` setzen oder einfach die `animation`-Property überschreiben. Wer eine andere Variante ausprobieren will: in den Keyframes die translate3d-Achse swappen (X statt Y für horizontal), Easing belassen — aber Scale meiden auf Productivity-Pages mit viel Text.

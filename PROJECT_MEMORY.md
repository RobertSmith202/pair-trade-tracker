...
- **Page-Transitions auf Desktop**: durchlief drei Iterationen während des Polish-Passes — Endstand seit Mai-2026 = **iPad-Home-Screen-Style** (220ms horizontaler Slide + Scale-Recede der leavenden Page + Cross-Fade, `cubic-bezier(0.22, 1, 0.36, 1)` Decelerate-Easing). Direction-aware via `data-direction`-Attribut. Mobile-Snap-Scroll unangetastet. Keyframes: `pageEnterDesktop` / `pageEnterDesktopPrev` (entering 48px-Slide + Scale 0.985→1.0 + Opacity 0→1), `pageLeaveDesktop` / `pageLeaveDesktopPrev` (leaving 12px in Gegenrichtung + Scale 1→0.97 + Opacity 1→0). translate3d für GPU-Compositing. Historische Iterationen:
  - Original: 280ms iOS-Control-Center-Vollscreen-Vertikal-Slide. War cinematic aber bremste spürbar nach 20× Wechsel; vertikal stimmte konzeptuell nicht zu den horizontal-angeordneten 4 Pages.
  - Iteration 2: 120ms reiner Opacity-Fade. Zu subtil — User las das als „kein Übergang", weil sub-Perzeptions-Schwelle und kein Richtungs-Cue.
  - Iteration 3 (current): 220ms iPad-Home-Style. Genug Bewegung für Direction-Cue, kurz genug für Productivity, Scale-Recede gibt subtilen Tiefen-Hinweis.
...
25. **Desktop-Page-Animation seit Polish-Pass-Iteration 3 = iPad-Home-Style** (`pageEnterDesktop` / `pageEnterDesktopPrev` / `pageLeaveDesktop` / `pageLeaveDesktopPrev`, 220ms, `cubic-bezier(0.22, 1, 0.36, 1)`). Horizontaler 48px-Slide auf kommender Page + Scale 0.985→1.0 + Opacity 0→1; leavende Page 12px in Gegenrichtung + Scale 1→0.97 + Opacity 1→0. Direction-aware: `data-direction="prev"` spiegelt die Vorzeichen, `data-direction="none"` deaktiviert die Animation komplett (für initial-load und Layout-Wechsel). Die JS-Sequenz in `activateDesktopPage` ist kritisch: `data-direction` setzen VOR `.active-page`-Klasse hinzufügen, plus `void incoming.offsetWidth` für Force-Reflow zwischen `classList.remove` und `add` damit die Animation auch bei Re-Navigation auf dieselbe Page neu feuert. Wer die Animation deaktivieren will: `data-direction="none"` setzen oder einfach die `animation`-Property überschreiben. Wer eine andere Variante will (z.B. vertikaler Slide oder reiner Fade): Keyframes ersetzen, Easing belassen.

...
- **Page-Transitions auf Desktop**: sechs Iterationen durchlaufen, Endstand seit Mai-2026 = **Bloomberg-Style Instant-Swap**, KEINE Animation. Aktive Page wird einfach durch z-index 2 sichtbar gemacht, leavende Page bleibt für 500ms (JS-Safety-Net-Cleanup) z-index 1 unsichtbar im DOM bis die Klasse weggeräumt wird. Begründung: Bloomberg-Philosophie „Speed = Elegance" — auf einem Trading-Tool ist jede Animation peripheres Rauschen, das System fühlt sich durch Geschwindigkeit eleganter an als durch Bewegung. Stripe Dashboard, IBKR, Bloomberg Terminal: alle so.
  ...
  - **Iteration 5** (200ms vertikaler Y-Slide + Cross-Fade): solide, modern, GPU-only, kein Wackeln. War technisch der Beste der animierten Varianten — aber Robert hat nach allem entschieden: gar keine Animation.
  - **Iteration 6 (current)**: Instant-Swap. Keine Keyframes, keine animation-Properties. JS-Toggle-Pattern (`.active-page` / `.leaving` / `data-direction`) bleibt unverändert für DOM-Compatibility — die leaving-Klasse hat einfach keine visuellen Auswirkungen mehr.
...
25. **Desktop-Page-Animation seit Iteration 6 = Bloomberg-Style Instant-Swap, KEINE Animation.** Nur display:block + z-index für `.active-page` (z:2) und `.leaving` (z:1, pointer-events:none). Die JS-Sequenz in `activateDesktopPage` (data-direction setzen, .active-page toggle mit Force-Reflow `void incoming.offsetWidth`, .leaving-Klasse + animationend-Cleanup mit 500ms-Safety-Net-setTimeout) ist defensiv unverändert geblieben. Das bedeutet: die leaving-Page sitzt für ~500ms als unsichtbarer z-index:1 Sibling im DOM nach jedem Wechsel, bis das Safety-Net-setTimeout sie wegräumt — kein visuelles oder interaktives Issue. Wer eine Animation wieder aktivieren will, kann die alten Keyframes aus Git-History rekonstruieren oder neue schreiben — der Aufruf-Mechanismus (data-direction + Force-Reflow) ist im JS schon da. **Warum keine Animation:** Bloomberg / IBKR / Stripe-Dashboard machen das alle so. Auf Productivity-Tools mit häufigem Page-Wechsel registriert das Brain Geschwindigkeit als Eleganz, nicht Bewegung. Iterationen 1-5 sind in der Animations-Sektion oben dokumentiert.

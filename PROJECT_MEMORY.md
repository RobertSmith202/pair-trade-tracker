...
7. **UI-Hinweis:** Auf der Pair- und Long-Page erscheint unter der Aggregate-Card ein dezenter Chip „💱 Rendite inkl. Wechselkurs vom Kauftag — …" (`.page-fx-note[data-fx-note-for="pair|long"]`), aber nur wenn tatsächlich mindestens ein Long-Leg in Fremdwährung existiert. Toggle in `refreshPageFxNotes()` am Ende von `render()`. Bei leerem PRICE_CACHE (Native-Ccy nicht auflösbar) wird vorsichtshalber gezeigt statt versteckt. Auf Short/Watch/Total-Pages absichtlich nicht — der Effekt existiert dort per Definition nicht.

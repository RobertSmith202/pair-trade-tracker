...
7. **UI-Hinweise (Chips unter Aggregate-Card):** drei Varianten, alle in `refreshPageFxNotes()` am Ende von `render()` toggled. Sichtbar nur wenn tatsächlich Fremdwährungs-Legs auf der Page existieren; bei leerem PRICE_CACHE (Native-Ccy nicht auflösbar) vorsichtshalber zeigen.
   - **Pair-Page:** `💱 Fremdwährungsrisiko ist in der GuV enthalten (Long-Seite)` — blau/info-tint.
   - **Long-Page:** `💱 Fremdwährungsrisiko ist in der GuV enthalten` — blau/info-tint.
   - **Short-Page:** `🔒 Kein Fremdwährungsrisiko — Cash bleibt in Fremdwährung` — grau/neutral (`.page-fx-note-neutral`). Begründung siehe Stolperfalle 28 unten (Consorsbank ohne USD-Sub-Konto = USD-Cash bleibt USD = kein FX-Effekt auf EUR-Rendite).
   - Watch/Total absichtlich ohne Chip (irrelevant dort).
...
**Gemini `thoughtSignature`-Fix (seit v2026-09-25.1):** Gemini 2.5+ verlangt, dass bei Multi-Turn-Function-Calling die vom Modell in Turn N zurückgegebene `thoughtSignature` beim entsprechenden `functionCall`-Part in Turn N+1 wieder mitgesendet wird — sonst HTTP 400 „Function call is missing a thought_signature in functionCall parts". Umsetzung: `geminiCall` extrahiert `p.thoughtSignature` aus jedem functionCall-Response-Part und persistiert sie als `_gemThoughtSignature` am `tool_use`-Block (privates Meta-Feld, Underscore-Präfix). `anthToGemContents` liest sie beim Rekonstruieren des History-Contents wieder aus und fügt sie am `functionCall`-Part hinzu. Wird transparent mit dem History-KV persistiert. Vor dem Fix (Sep 23-25 2026) fiel der Bot bei jedem Tool-Call-Turn stumm auf Anthropic zurück (kostete ~2-5 Cent pro Dialog statt 0).
...
28. **FX-Snapshot ist NUR für Long-Legs implementiert — bewusste Asymmetrie, wirtschaftlich sauber begründet.** Bei Robert (Consorsbank ohne USD-Sub-Konto) bleibt der Short-Erlös als Fremdwährungs-Cash liegen (USD → USD-Cash → USD-Buyback). Da entsteht kein FX-Effekt auf die EUR-Rendite: der Wertverlust des Fremdwährungs-Cash und der günstigere Fremdwährungs-Buyback saldieren sich mathematisch heraus. Formel `pnlInEntry × aktueller_FX` liefert die richtige EUR-Zahl. Bei Long fließt dagegen echt EUR raus (Kauf einer Fremdwährungs-Aktie), also entsteht ein Cross-Border-FX-Effekt → historisiert. Wer die Symmetrie in `computeLeg`/`legPnl` aufhebt und Short-Legs auch historisiert, würde bei Roberts Setup den FX-Effekt DOPPELT zählen — vorher explizit mit ihm klären, ob sich sein Broker-Setup geändert hat (USD-Sub-Konto vs. reines EUR-Konto).
